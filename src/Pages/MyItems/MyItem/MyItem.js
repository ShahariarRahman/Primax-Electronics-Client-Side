...
const MyItem = ({ item, deleteInventory }) => {
    ...
    return (
        <tr className='border-t'>
            ...
            <td className="px-1 sm:px-6 py-4 text-sm text-gray-500 break-all text-center">Supplier: {supplier} <br /> Price: {price} <br /> Description: {description}
            </td>
            <td className="text-sm text-center md:text-right px-1 sm:px-2 py-2">
                <div >
                    ...
                    <button onClick={() => deleteInventory(_id)} className='sm:ml-2 border border-gray-100 w-20 h-8 my-1 bg-red-500 text-white rounded-md'>Cancel</button>
                </div>
            </td>
        </tr>
    );
